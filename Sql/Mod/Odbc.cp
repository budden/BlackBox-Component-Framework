...
**)
	...
	*)
	...
	PROCEDURE Check (ret: INTEGER; d: Driver; stmt: WinSql.HSTMT; OUT res: INTEGER);
		VAR err: INTEGER; len: SHORTINT; state, msg: ARRAY 256 OF CHAR; dbc: WinSql.HDBC;
	BEGIN
		CASE ret OF
		| WinSql.SQL_SUCCESS:
			res := 0
	...
	BEGIN
		...
		ASSERT(t.stmt # 0, 22);
		Check(WinSql.SQLDescribeColW(t.stmt, SHORT(column + 1), name, LEN(name),
	...
	PROCEDURE Open* (id, password, datasource: ARRAY OF CHAR; async, showErr: BOOLEAN;
		...
		VAR h: Driver; r: INTEGER; len: SHORTINT;
	BEGIN
		NEW(h); d := NIL;
		...
		IF res = WinSql.SQL_SUCCESS THEN
			...
			Check(WinSql.SQLSetConnectOptionW(h.dbc, WinSql.SQL_QUIET_MODE, HostWindows.main), h, 0, r);
			IF res = 0 THEN
				...
				IF res = 0 THEN
					h.Init("?");
					Check(WinSql.SQLGetInfoW(h.dbc, WinSql.SQL_FETCH_DIRECTION,
															SYSTEM.ADR(h.directions), 4, len), h, 0, r);
					(* ODBC default is SQL_AUTOCOMMIT_ON *) 
					IF ~autoCommit THEN
						Check(WinSql.SQLSetConnectOptionW(h.dbc, WinSql.SQL_AUTOCOMMIT,
																				WinSql.SQL_AUTOCOMMIT_OFF), h, 0, r);
					END;
					Check(WinSql.SQLSetConnectOptionW(h.dbc, 1200+4, 1), h, 0,r);	(* set Preserve Cursors ON *)
					h.showErr := showErr;
