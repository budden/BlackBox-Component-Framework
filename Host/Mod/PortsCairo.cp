...
(**
	...
	contributors	= "Oberon microsystems, Marco Ciot, Alexander Iljin, Ivan Denisov, Dmitry Solomennikov"
...
**)
	...
	IMPORT
		SYSTEM, WinApi, Kernel, Fonts, Ports, Dialog, Services, HostFonts, C, Cwin;
...
*)
	...
	PROCEDURE (rd: Rider) DrawOval* (l, t, r, b, s: INTEGER; col: Ports.Color);
		VAR
			...
			surface: C.cairo_surface_t; ctx: C.cairo_t; rad : INTEGER;
	BEGIN
		ASSERT(rd.port # NIL, 100);
		p := rd.port; dc := p.dc;
		IF p.wnd = 0 THEN
	...
	PROCEDURE (rd: Rider) DrawLine* (x0, y0, x1, y1, s: INTEGER; col: Ports.Color);
		VAR res: INTEGER; pt: WinApi.POINT; p: Port; dc, oldb, oldp: WinApi.HANDLE;
			surface: C.cairo_surface_t; ctx: C.cairo_t;
	BEGIN
		ASSERT(s >= 0, 20);
		ASSERT(rd.port # NIL, 100);
		p := rd.port; dc := p.dc;
